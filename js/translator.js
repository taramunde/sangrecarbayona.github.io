...
(function () {
  ...
  const TRANSLATIONS = {
    ...
    /* ---- Hero / cabecera ---- */
    "desde 1926, con orgullo, valor y garra.":
      "Since 1926, with Pride, Courage and Grit.",
    ...
    "la enciclopedia del fútbol español":
      "The Encyclopaedia of Spanish Football",
    "datos históricos y estadísticas": "Historical data and statistics",
    "la web del fútbol modesto": "The grassroots football website",
    "diario digital independiente del fútbol asturiano":
      "Independent digital newspaper of Asturian football",
    "la academia del fútbol español": "The Spanish Football Academy",
    "fotos de formaciones de equipos de fútbol y selecciones nacionales":
      "Photos of football team line-ups and national teams",
    "trayectoria de jugadores": "Player career records",
    "un paseo virtual por las equipaciones de equipos de fútbol de todos los tiempos":
      "A virtual tour of football kits from all eras",
    ...
    "el real oviedo como nunca lo viste":
      "Real Oviedo like you've never seen it before",
    "juega, descubre y revive la historia":
      "Play, discover and relive the history",
    ...
    "alineaciones, fichas detalladas de jugadores, resultados y el mejor contenido para la afición carbayona":
      "Line-ups, detailed player profiles, results and the best content for the Carbayona fans",
  };
  ...
  function isProtected(text) {
    ...
    const exactMatch = PROTECTED_EXACT.some(
      (term) => tLower === term.toLowerCase(),
    );
    // Contiene: el nodo incluye una de estas frases largas (títulos de libros, etc.)
    const containsMatch = PROTECTED_CONTAINS.some((term) =>
      tLower.includes(term.toLowerCase()),
    );
    ...
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode(node) {
          ...
          if (isInNoTranslateBlock(node)) return NodeFilter.FILTER_REJECT;
          // Ignorar texto vacío
